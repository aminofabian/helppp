...
//   </div>
//   </div>
//   <p className="dark:text-slate-50">Spreading Kindness, One Act at a Time: Join Us in Making a Difference Today!.</p>
//   <Separator className='my-5' /> 
//   <div className='flex flex-col justify-center flex-shrink gap-y-5 dark:text-slate-200'>
//   <Button className="dark:text-slate-50"asChild>
//   <Link href='/c/eldoret/create'>
//   Create a Help Request
//   </Link>
//   </Button>
...
export default async function HomeNavRight() {
  ...
  return (
    ...
    <Card>
    <div className="max-w-xs p-6 rounded-md shadow-md dark:bg-gray-50 dark:text-gray-900">
    <h1 className='text-xs font-light text-primary bg-green-50 p-2 rounded-lg shadow mx-auto flex justify-center gap-1'>
    Welcome aboard {user.given_name || user.family_name ? (
      <>
      {user.given_name && (
        <span className="capitalize">{user.given_name.toLowerCase()} </span>
      )}
    ...
    <div className="stats bg-primary text-primary-content mt-4 flex flex-col w-full">
    <div className="stat p-2">
    <div className="stat-title text-xs text-slate-300 uppercase">Account balance</div>
    <div className="stat-value text-lg">{wallet ? `${wallet.balance} KES` : '0 KES'}</div>
    <div className="stat-actions mt-2">
    <Dialog>
    <DialogTrigger asChild>
    <button className="btn btn-xs btn-success w-full text-slate-200">Add funds</button>
    </DialogTrigger>
    <DialogContent>
    <DialogHeader>
    ...
    <div className="stat p-2">
    <div className="stat-title text-xs text-slate-300 uppercase">Current balance</div>
    <div className="stat-value text-lg">{wallet ? `${wallet.balance} KES` : '0 KES'}</div>
    <div className="stat-actions mt-2 flex justify-between">
    <Dialog>
    <DialogTrigger asChild>
    <button className="btn btn-xs flex-1 mr-1">Withdrawal</button>
    </DialogTrigger>
    <DialogContent>
    <DialogHeader>
    ...
    <button className="btn btn-xs flex-1 ml-1">Deposit</button>
    </div>
    </div>
    </div>
    <Separator className='my-5' /> 
