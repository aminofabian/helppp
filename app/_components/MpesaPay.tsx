...
const stkPushQueryWithIntervals = async (CheckoutRequestID: string) => {
  ...
  console.log(CheckoutRequestID, 'Checking MPesa payment status');
  const timer = setInterval(async () => {
    ...
    if (requestCount >= maxRetries) {
      clearInterval(timer);
      setIsLoading(false);
      toast.error("Timeout: Payment not completed in time.");
      return;
    }
    ...
    try {
      ...
      if (data?.ResultCode === "0") {
        ...
        clearInterval(timer);
        setIsLoading(false);
        toast.error(data?.ResultDesc || "Payment not completed.");
      }
    } catch (error) {
      clearInterval(timer);
      setIsLoading(false);
