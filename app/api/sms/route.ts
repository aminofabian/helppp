  ...
  try {
    ...
    const emailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; padding: 20px; border: 1px solid #eee; border-radius: 5px;">
        ...
        <p>Hello ${userName},</p>
        <p>${isSuccessful ? `Your withdrawal of <strong>${formattedAmount}</strong> has been processed successfully.` : `Your withdrawal request of <strong>${formattedAmount}</strong> has been received.`}</p>
        <p>Need help? Contact us at support@fitrii.com</p>
      </div>
    `;

    const subject = 'Withdrawal Notification';
    const { getUser } = getKindeServerSession();
    const user = await getUser();
    const recipient = user?.email || 'default@example.com';

    const senderEmail = process.env.EMAIL || 'noreply@fitrii.com';
