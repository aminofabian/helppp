...
//    let giverWallet = await prisma.wallet.findUnique({ where: { userId: giver.id } });
//    console.log(giverWallet, 'giverwallet...')
//   if (!giverWallet) {
//   giverWallet = await prisma.wallet.create({
//     data: { userId: giver.id, balance: 0 }
//   });
//   console.log(`New wallet created for giver: ${giver.id}`);
// }
...
// });
// console.log(`Giver's wallet updated: ${giver.id}, Amount deducted: ${amount}`);
...
//       const pointsEarned = Math.floor(amount / 50);
//       await prisma.points.create({
//         data: { userId: user.id, amount: pointsEarned, paymentId: payment.id }
//       });

//       const totalPoints = await prisma.points.aggregate({ where: { userId: user.id }, _sum: { amount: true } });
//       const newLevel = calculateLevel(totalPoints._sum.amount || 0);
