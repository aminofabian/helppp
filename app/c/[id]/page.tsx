...
export default async function CommunityPage({params}: {params: {id: string}}) {
  ...
  return (
    ...
    <div className="container h-fit rounded-lg bg-gray-200 my-5 dark:bg-slate-800">
    ...
    <Card>
    <CardTitle className='text-lg py-4 ml-5'>About {params.id} Community</CardTitle>
    </Card>
    ...
    <LinkBreak2Icon /> Created: {new Date(data?.createdAt as Date).toLocaleDateString('en-us', {
      weekday: 'long',
      year: 'numeric',
      month: 'short',
      day: 'numeric'
    ...
    <DropdownMenuSeparator className='my-4 text-bold'/>
    <Button asChild><Link href={user?.id ? `/c/${data?.name}/create` : "/api/auth/login"}>Create a Request</Link></Button>
    </div>
    </div>
    <div>
    <img src="https://source.unsplash.com/random/100x100/?5" alt="" className="object-cover w-full mb-4 h-30 sm:h-96 dark:bg-gray-500" />
    ...
    {user?.id === data?.userId ? (
      ...
      ): <Button className = 'text-sm font-semibold text-slate-50 px-5'>Join Community</Button>}
